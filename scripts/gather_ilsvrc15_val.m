...
for p = 1:length(ps) 
  ...
  aa = load(['../msc/ilsvrc15_val_', num2str(first_el), '_', num2str(last_el), '.mat']);
  ...
  convert_mat_to_txt(aa.boxes, aa.images, 'val');
...
%  fprintf('first_el: %d, last_el: %d\n', first_el, last_el);
%
%  bb = load(['../msc/ilsvrc15_val_msc_', num2str(first_el), '_', num2str(last_el), '.mat']);
%  bb.gray_img_indices = bb.img_indices(bb.gray_img_indices);
%  if isfield(bb, 'large_img_indices')
%    bb.large_img_indices = bb.img_indices(bb.large_img_indices);
%  else
...
%fprintf('size(gray_img_indices, 1): %d, size(large_img_indices, 1): %d\n', size(gray_img_indices, 1), size(large_img_indices, 1)); 
%
%%save(['ilsvrc_2015_val.mat'], 'boxes', 'images', '-v7.3');
%save(['ilsvrc_2015_msc_val.mat'], 'gray_img_indices', 'img_indices', 'large_img_indices');
%
